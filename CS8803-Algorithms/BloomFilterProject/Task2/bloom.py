...
class HashType1(object):
    ...
    def getHashList(self, x):
        ...
        for i in range(self.k):
            random.seed((self.seeds[i] + x) % 2147483647)
            results.append(random.randint(low=0, high=((self.k * self.m) - 1)))
...
class HashType2(object):
    ...
    def getHashList(self, x):
        ...
        for i in range(self.k):
            results.append((((self.a[i] * x) + self.b[i]) % self.P) % self.n)
