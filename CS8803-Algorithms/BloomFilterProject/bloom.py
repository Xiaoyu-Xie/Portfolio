...
class HashType1(object):
    ...
    def __init__(self, config, genHashes):
        ...
        # self.hashtables = [[0 for x in range(self.k)] for y in range(self.n)]
        self.hashtables = list()
        self.originalvalues = list()
        ...
        if genHashes :
            ...
            self.seeds = random.randint(low=0, high=(self.n-1), size=self.k)
            pass
    ...
    def getHashList(self, x):
        ...
        random.seed((self.seeds[0] + x) % 2147483646)
        self.hashtables.append(random.randint(low=0, high=(99)))
        self.originalvalues.append(x)
            # self.hashtables[res[i]][i] += 1
            # self.hashtables[res[i]] += 1
...
class HashType2(object):
    ...
    def __init__(self, config, genHashes):
        ...
        if genHashes :
            ...
            self.a = random.randint(config['genSeed'])
            self.b = random.randint(config['genSeed'])
    ...
    def getHashList(self, x):
        ...
        self.hashtables.append((((self.a * x) + self.b) % self.P) % 99)
...
def main():	
    ...
    parser.add_argument('-i', '--infile',  help='Input file of data to add to Bloom Filter', default='orderedInput.txt', dest='inFileName')
    ...
    parser.add_argument('-t', '--task',  help='Which task to perform (1,2,3)', type=int, choices=[1, 2, 3], default=1, dest='taskToDo')
