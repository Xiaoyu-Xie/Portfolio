#include "gfserver.h"/* note that the -n and -z parameters are NOT used for Part 1 */
...
"  -t [thread_count]   Num worker threads (Default: 2, Range: 1-512)\n"              \
"  -s [server]         The server to connect to (Default: Udacity S3 Data)\n"        \
"  -z [segment_size]   The segment size (in bytes, Default: 5000).\n"                \
...
extern void initializeProxyMutexes();
extern void initializeProxySharedMemory();
extern void termHandler(int signo);
...
int main(int argc, char **argv) {
  ...
  while ((option_char = getopt_long(argc, argv, "x:n:p:s:t:z:h", gLongOptions, NULL)) != -1) {
    switch (option_char) {
      default:
 ...
 if (port < 1024) {
  ...
  printf("PROXY: Initializing Mutexes\n");
  initializeProxyMutexes(nsegments);
  printf("PROXY: Initializing Shared Memory and Queues\n");
  initializeProxySharedMemory(segsize);
